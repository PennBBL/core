...
def test_project_template(data_builder, file_form, as_user, as_admin):
    ...
    assert as_user.post('/acquisitions/' + acquisition_2 + '/files', files=file_form('non-compliant.txt')).ok
    assert as_user.post('/acquisitions/' + acquisition_2 + '/files', files=file_form('compliant1.csv')).ok
    assert as_user.post('/acquisitions/' + acquisition_2 + '/files', files=file_form('compliant2.csv')).ok
    assert as_user.post('/acquisitions/' + acquisition_2 + '/files/compliant1.csv/classification', json={'add': {'custom': ['diffusion']}})
    ...
    r = as_user.post('/projects/' + project + '/template', json={
    ...
    r = as_user.post('/projects/' + project2 + '/template', json={
        'session': {'subject': {'code': '^compliant$'}},
        'acquisitions': [{
    ...
    assert as_user.put('/sessions/' + session, json={'subject': {'code': 'non-compliant'}}).ok
    assert not satisfies_template()
    assert as_user.put('/sessions/' + session, json={'subject': {'code': 'compliant'}}).ok

    # test that moving session to another project correctly updates session.satisfies_template
    assert satisfies_template()
    assert as_user.put('/sessions/' + session, json={'project': project2})
    assert not satisfies_template()
    assert as_user.put('/sessions/' + session, json={'project': project})
    assert satisfies_template()

    # test moving session to project without template
    assert as_user.delete('/projects/' + project2 + '/template')
    r = as_user.put('/sessions/' + session, json={'project': project2})
    assert r.ok
    r = as_user.get('/sessions/' + session)
    assert r.ok
    assert 'project_has_template' not in r.json()
